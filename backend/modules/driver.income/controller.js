    ...
    try {
      ...
      if (!income) {
        ...
        if (income.status == "rejected") {
          const driverEmail = income.driverEmail;
          const sub = "Income Rejected";
          const message = `<p>Your income with id ${income._id} has been <strong>rejected</strong> because : <br/> <strong> ${income.rejectionReason}</strong></p>`;
          sendMail(driverEmail, sub, message);
        }else if (income.status == "approved"){
          const driverEmail = income.driverEmail;
          const sub = "Income Approved";
          const message = `<p>Your income with id ${income._id} has been <strong>approved</strong></p>`;
          sendMail(driverEmail, sub, message);
        }
      }
      res.status(200).json(income);
